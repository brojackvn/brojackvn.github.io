    ...
    function mergeFromYMLFrontmatter(target, source) {
        ...
        if (source.publishedDate) {
            if (source.publishedDate instanceof Date) {
                ...
                console.error(
                    "Don't know what to do with published date: " +
...
`
    ...
    function _moveLegacyAffiliationFormatIntoArray(frontMatter) {
        ...
        for (let author of frontMatter.authors) {
            ...
            if (hasNewStyle) {
                ...
                if (author.affiliationURL)
                    newAffiliation.url = author.affiliationURL
        ...
        listeners: {
            onCiteKeyCreated(event) {
                ...
                const numbers = keys.map((key) =>
                    frontMatter.citations.indexOf(key)
                )
                citeTag.numbers = numbers
                const entries = keys.map((key) =>
                    frontMatter.bibliography.get(key)
                )
            ...
            onCiteKeyChanged() {
                ...
                // update bibliography
                const citationListTag =
                    document.querySelector('d-citation-list')
                const bibliographyEntries = new Map(
                    frontMatter.citations.map((citationKey) => {
                        return [
                            citationKey,
                            frontMatter.bibliography.get(citationKey),
                        ]
                    })
                )
            ...
            onBibliographyChanged(event) {
                ...
                if (citationListTag.hasAttribute('distill-prerendered')) {
                    console.debug(
                        'Citation list was prerendered; not updating it.'
                    )
                } else {
            ...
            onFrontMatterChanged(event) {
                ...
                const prerendered = document.body.hasAttribute(
                    'distill-prerendered'
                )
            ...
            DOMContentLoaded() {
                if (Controller.loaded) {
                    ...
                    console.debug(
                        'Runlevel 4: Controller running DOMContentLoaded'
                    )
                }
    ...
    function addPolyfill(polyfill, polyfillLoadedCallback) {
        ...
        script.onerror = function () {
            new Error(
                'Runlevel 0: Polyfills failed to load script ' + polyfill.name
            )
        }
    ...
    class Polyfills {
        ...
        static load(callback) {
            ...
            const polyfillLoaded = function (polyfill) {
                ...
                console.debug(
                    'Runlevel 0: Polyfill has finished loading: ' +
                        polyfill.name
                )
                            ...
                            case '#text':
                                {
                                    ...
                                    if (!isOnlyWhitespace.test(text)) {
                                        ...
                                        addedNode.parentNode.removeChild(
                                            addedNode
                                        )
                                    }
                                }
    ...
    class Bibliography extends HTMLElement {
        ...
        parseIfPossible() {
            ...
            if (scriptTag.type == 'text/bibtex') {
                ...
                console.warn(
                    'Unsupported bibliography script tag type: ' +
                        scriptTag.type
                )
            }
        }
...
`
    ...
    class HoverBox extends T$7(HTMLElement) {
        ...
        showAtNode(node) {
            ...
            this.show([
                node.offsetLeft + bbox.width,
                node.offsetTop + bbox.height,
            ])
        }
...
`
    ...
    class Interstitial extends T$9(HTMLElement) {
        ...
        shouldRemoveSelf() {
            ...
            if (typeof Storage !== 'undefined') {
                if (
                    localStorage.getItem(this.localStorageIdentifier()) ===
                    'true'
                ) {
                    console.log(
    ...
    function drag() {
        ...
        function beforestart(id, container, point, that, args) {
            ...
            if (
                !customEvent(
                    ...
                    function () {
                        if (
                            (event.subject = s = subject.apply(that, args)) ==
                            null
                        )
                            return false
                        dx = s.x - p[0] || 0
        ...
        drag.filter = function (_) {
            return arguments.length
                ? ((filter = typeof _ === 'function' ? _ : constant$3(!!_)),
                  drag)
                : filter
        }
        ...
        drag.container = function (_) {
            return arguments.length
                ? ((container = typeof _ === 'function' ? _ : constant$3(_)),
                  drag)
                : container
        }
        ...
        drag.subject = function (_) {
            return arguments.length
                ? ((subject = typeof _ === 'function' ? _ : constant$3(_)),
                  drag)
                : subject
        }
        ...
        drag.touchable = function (_) {
            return arguments.length
                ? ((touchable = typeof _ === 'function' ? _ : constant$3(!!_)),
                  drag)
                : touchable
        }
...
`
    ...
    const initialize = function () {
        ...
        for (const component of allComponents) {
            console.debug(
                'Runlevel 2: Registering custom element: ' + component.is
            )
